...
const MenuItem = ({ path, label, onClick }) => (
  <li>
    {/* npm install react-router-dom 후 사용 가능  */}
    <Link
      to={path}
...
const navbar = () => {
    ...
    return (
        <nav className="fixed top-0 left-0 w-full bg-white font-black p-4 shadow-lg z-50">
        <div className="container mx-auto flex justify-between items-center">
            <h1 className="text-lg lg:text-2xl font-bold lg:ml-12 lg:mr-8">
            ...
            {/* className="hidden"은 1024픽셀에서만 보이게됨(데스크톱) */}
            <select value={language} onChange={(e) => setLanguage(e.target.value)} className="hidden lg:block px-3 ml-8 border round-md bg-white hover:border-blue-500 transition duration-300">
                <option value="ko">한국어</option>
                <option value='en'>영어</option>
            </select>
            {/* className="lg:hidden"은 1024픽셀아래에서만 보임(모바일) */}
            <button className="lg:hidden text-2xl" onClick={toggleMenu} aria-lavel="메뉴">
                {isOpen ? <HiX/> : <HiMenu/>}
            </button>
        </div>
        <div className={`fixed top-0 right-0 h-full w-64 bg-white text-black transform transition-transform duration-300 ease-in-out z-50 ${isOpen ? "translate-x-0":"translate-x-full"} lg:hidden`}>
            <div className="p-4">
                ...
                <ul className="clear-both space-y-4 pt8 text-lg">
                    {menuItems.map((item) => (
